 ...
 * - Reads the user's stream row + playlist from Supabase
 * - Downloads videos from the private `videos` storage bucket
 * - Pushes a continuous RTMP feed to YouTube via FFmpeg
 * - Loops the playlist forever and auto-reconnects on failure
 *
 * Required env vars:
 *   SUPABASE_URL
 *   SUPABASE_SERVICE_ROLE_KEY
 *   USER_ID                 (uuid of the user whose stream to run)
 *   STORAGE_BUCKET          (default: "videos")
...
async function downloadPlaylist(videos) {
  ...
  for (let i = 0; i < videos.length; i++) {
    ...
    if (error) throw new Error(`Download failed for ${v.storage_path}: ${error.message}`);
...
async function runStream(stream) {
  ...
  await setStatus("starting", { last_started_at: new Date().toISOString(), last_error: null });

  const videos = await getPlaylist();
  if (!videos.length) {
    await setStatus("error", { last_error: "No videos in playlist" });
    return;
  }
      ...
      if (!stream) {
        ...
        if (wantsLive && (!activeRun || key !== lastKey)) {
          ...
          activeRun = runStream(stream).catch(async (err) => {
            ...
            await setStatus("error", { last_error: err.message });
          });
